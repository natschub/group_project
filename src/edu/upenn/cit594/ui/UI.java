...
public class UI {
	...
	private String specifyADate(){
		...
		System.out.println("Which day are you searching for? Please enter a date in the format: YYYY-MM-DD.");
	...
	private String specifyAZipcode(){
		...
		System.out.println("Which area are you searching for? Please enter a 5-digit Zipcode.");
	...
	private void showVacciPerCapita() {
		...
		if(populationFile != null && covidFile != null) {
			System.out.println("Partial or full vaccinations per capita? Please type \"partial\" or \"full\". \n > ");
			String str = in.nextLine();
			System.out.flush();
			if (str.toLowerCase().equals("partial") || str.toLowerCase().equals("full") ) {
				
				Map<String, Double> vacciMap =  processor.getVaccinationsPerCapita(str, specifyADate());  
				System.out.println("BEGIN OUTPUT");
				if(vacciMap == null) {
					System.out.println(0);
				}else {
	...
	private void showAveragePropertyMarketValue() {
		...
		if(propertiesFile != null) {
			...
			System.out.println("BEGIN OUTPUT");
			int averageMarketValue = processor.getAverageMarketValue(specifyAZipcode());
	...
	private void showAverageTotalLivableArea() {
		...
		if(propertiesFile != null) {
			...
			System.out.println("BEGIN OUTPUT");
			int averageLivable= processor.getAverageLivableArea(specifyAZipcode());
	...
	private void showTotalPropertyMarketValue() {
		...
		if(populationFile != null && propertiesFile != null) {
			System.out.println("BEGIN OUTPUT");
			int totalPropertyValue = processor.getMarketValuePerCapita(specifyAZipcode());
